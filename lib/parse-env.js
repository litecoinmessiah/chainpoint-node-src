...
let envDefinitions = {
  ...
  CHAINPOINT_NODE_PUBLIC_URI: envalid.url({
    ...
    desc: 'The scheme and publicly accessible IP address for this Node. Use private range IP to skip advertising publicly.',
    example: 'http://0.0.0.0'
  }),

  CHAINPOINT_NODE_PRIVATE_URI: envalid.str({
    default: '',
    desc: 'A URI with a Private (RFC1918) IPv4 Address used to expose additional metadata when submitting a POST /hashes.'
  }),

  CHAINPOINT_NODE_REFLECTED_URI: envalid.str({
    default: '',
    desc: 'An enumerated data type consisting of two enumerals (public|private). Used in conjunction with CHAINPOINT_NODE_PUBLIC_URI or CHAINPOINT_NODE_PRIVATE_URI to provide additional metadata when submitting a POST /hashes.'
  }),

  CHAINPOINT_NODE_SEND_USAGE_STATS: envalid.str({
    default: 'disabled',
    desc: 'You can optionally opt into sending Node usage data to Chainpoint.'
  }),

  CHAINPOINT_NODE_SEND_USAGE_STATS_TELEGRAF_URI: envalid.str({
    default: '',
    desc: 'You can optionally opt into sending Node usage data to Chainpoint.'
  }),
