...
CONFIG_DIR=$HOME/.chainpoint/node
echo $CONFIG_DIR
mkdir -p $CONFIG_DIR
...
[ -f ./cert.key ] && \
  chmod 770 $CONFIG_DIR/$FILE_NAME.* && \
  rm $CONFIG_DIR/$FILE_NAME.* && \
...
openssl req -new -x509 -newkey rsa:4096 -sha256 -nodes -keyout "$CONFIG_DIR/$FILE_NAME.key" -days $DAYS -out "$CONFIG_DIR/$FILE_NAME.crt" -passin pass:$PASSPHRASE -config "$CONFIG_DIR/$CONFIG_FILE"

# Store the human readable details of the generated crt in *.info file
openssl x509 -noout -fingerprint -text < "$CONFIG_DIR/$FILE_NAME.crt" > "$CONFIG_DIR/$FILE_NAME.info"

# Protect the key
chmod 444 "$CONFIG_DIR/$FILE_NAME.key"
